...
class MyLayernorm(torch.nn.Module):
    def __init__(self, run_type = 'nn', layernorm_dict = None, dtype='fp32'):
        super(MyLayernorm, self).__init__()
        self.run_type = run_type

        if self.run_type == 'nn' and (layernorm_dict is None) :
            raise Exception('Please enter the proper data file')

        if self.run_type == 'nn':
            self.n_neuron_sqrt = layernorm_dict['n_neurons']
            self.nn_sqrt=  ApproximatorNN(layernorm_dict['n_neurons'])
            self.nn_sqrt.load_state_dict(layernorm_dict['state_dict'])
        ...
        #        self.nn_div.half()
            ...
            self.lut_sqrt = LUT(layernorm_dict, dtype)
